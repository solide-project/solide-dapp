...
export const items = async (): Promise<ContractSchema[]> => {
  const response = await fetch(
    "https://raw.githubusercontent.com/CallistoEnterprise/callisto-wallet-dev/main/app/scripts/tokens/cloTokens.json"
  )
  const data: { address: string; decimal: number; symbol: string }[] =
    await response.json()
  ...
  data
    ...
    .forEach((token) => {
      ret.push({
        ...
        tutorial: "",
        reference:
          "https://github.com/CallistoEnterprise/callisto-wallet-dev/blob/main/app/scripts/tokens/cloTokens.json",
        playground: {
          default: {
