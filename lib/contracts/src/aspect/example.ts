...
const examples: any[] = [{
    ...
    aspect: ["aspect/guard-by-count.ts", "aspect/guard-by-lock.ts", "aspect/guard-by-trace.ts"],
},
{
    key: "operation",
    ...
    examples.forEach((example: any) => {
        example.aspect.forEach((aspect: string) => {
            ret.push({
                ...
                tutorial:
                    `https://github.com/artela-network/aspect-example/blob/main/${example.key}/README.md`,
                reference:
                    `https://github.com/artela-network/aspect-example/blob/main/${example.key}`,
                description:
                    `Artela Aspect Example for ${example.description}`,
                playground: {
                    default: {
                        address:
                            `https://github.com/artela-network/aspect-example/blob/main/${example.key}/${aspect}`,
                    },
                },
                type: "aspect",
            })
