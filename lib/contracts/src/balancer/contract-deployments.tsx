...
export const items = async (chain: string) => {
    ...
    const activeObjects = Object.values(data).filter((obj: any) => obj.status === "ACTIVE")
    activeObjects.forEach((obj: any) => {
        obj.contracts
            .filter((contract: Contract) =>
                !(contract.name.toLowerCase().includes("factory") || contract.name.toLowerCase().includes("proxy")))
