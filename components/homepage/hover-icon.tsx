...
interface HoverIconsProps extends React.HTMLAttributes<HTMLDivElement> {
  background?: string
  images?: string[]
  text?: string
}
...
export const HoverIcons = ({
        ...
        switch (index) {
          case 0:
            ...
            ; (icon as HTMLElement).style.transform =
              `translate3d(${0.6 * dimension.width}px, ${(1 / 2) * dimension.height - 28}px, 0)`
            break
          case 1:
            // TR
            ; (icon as HTMLElement).style.transform =
              `translate3d(${0.8 * dimension.width}px, ${2 * -dimension.height}px, 0px)`
            break
          case 2:
            // BL
            ; (icon as HTMLElement).style.transform =
              `translate3d(${-0.3 * dimension.width}px, ${-0.5 * dimension.height}px, 0px)`
            break
          case 3:
            // BBL
            ; (icon as HTMLElement).style.transform =
              `translate3d(${-0.2 * dimension.width}px, ${0.8 * dimension.height}px, 0px)`
            break
          case 4:
            // TL
            ; (icon as HTMLElement).style.transform =
              `translate3d(${-0.5 * dimension.width}px, ${-2 * dimension.height}px, 0px)`
            break
          case 5:
            // MR
            ; (icon as HTMLElement).style.transform =
              `translate3d(${1.16 * dimension.width}px, ${-0.7 * dimension.height}px, 0px)`
            break
        }
      }
    })
  ...
  return (
    ...
    >
      <span
        className={cn("flex py-2 px-3 rounded-lg tilt-div mx-2", background)}
      >
        {text}
      </span>
